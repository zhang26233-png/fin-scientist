...
FinScientist V0.9.2b 是一个单文件 Streamlit 金融研究学习原型。它支持美股、港股、A股，提供单股票行情分析、技术指标、基本面字段、板块观察、新闻/事件分析、多股票对比、临时自选股观察列表、简单策略回测、数据源可靠性与数据质量报告，并增强自动研究对象筛选模块的 A股批量行情获取稳定性。
...
- A股：主数据源为 `AkShare`，筛选模块备用数据源为 `yfinance`。查询前会转换为 6 位数字代码。AkShare 可能因接口返回空数据、字段变化、日期范围、代理/VPN、请求频率或网络环境导致失败。
...
获取失败表展示无法获取行情的候选对象及诊断信息，包括股票代码、实际查询代码、市场、数据源、尝试参数、失败阶段、网络诊断和失败原因。
