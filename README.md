...
FinScientist V0.9.1 是一个单文件 Streamlit 金融研究学习原型。它支持美股、港股、A股，提供单股票行情分析、技术指标、基本面字段、板块观察、新闻/事件分析、多股票对比、临时自选股观察列表、简单策略回测、数据源可靠性与数据质量报告，并新增自动研究对象筛选模块骨架。
