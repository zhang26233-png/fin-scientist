...
Final ranking is based on `activated_composite_score` descending. If that score is missing, the Scheduler uses a neutral fallback score:

```text
0.30 * fundamental_research_score
...
Missing component scores use a neutral value of 50 so the page does not crash when a data source is incomplete.

Core Research contains the top-ranked research candidates within the Core limit when `activated_composite_score >= 55` and severe data-quality issues are absent. Watch Research contains the next ranked candidates within the Watch limit or objects with `activated_composite_score >= 50`. Excluded / Low Priority contains the remaining objects and rows with severe unavailable data, suspension/ST/delisting risk, or invalid price.

If absolute thresholds are too strict and fewer than five Core rows are produced, the Scheduler uses relative-ranking fallback and promotes the highest-ranked valid rows into Core Research. In that case `research_scheduler_warning` records: "Core generated by relative ranking fallback because absolute threshold was too strict." All outputs remain only for learning and research and do not constitute investment advice.
